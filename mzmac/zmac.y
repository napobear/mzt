...
// mzmac version reverted to major minor numbering
 ...
 *
 * 20140710	Forked by original zmac code.
 * 		Reverted some commamd line option and features to zmac 1.4
 * 		style.
 * 		Output formats made selectable by user (binary as default).
 */
...
unsigned int mfread(char *ptr, unsigned int size, unsigned int nitems, FILE *f);
unsigned int mfwrite(char *ptr, int size, int nitems, FILE *f);
...
#define MALLOC_SIZE 	10000		// for mio funcs
...
int	relopt = 0;	// Output .rel files.
int	amsopt = 0;	// Output .ams files.
int	bdsopt = 0;	// Output .bds files.
int	casopt = 0;	// Output .cas files.
int	binopt = 0;	// Output .bin files. (default)
int	cmdopt = 0;	// Output .cmd files.
int	hexopt = 0;	// Output .hex files.
int	lcasopt = 0;	// Output .lcas files.
...
unsigned char *mhead;			/* pointer to start of malloc()d area */
unsigned char *mend;			/* pointer to current (just beyond) EOF*/
unsigned char *mptr;			/* pointer to current position */
unsigned int msize;			/* size of chunk mhead points to */
...
char	bopt = 1,
	copt = 0,	/* no cycle counts in listings by default */
	edef = 1,
	eopt = 1,
	fdef = 0,
	fopt = 0,
...
/*
 *  put out a byte of binary in INTEL standard form
 */
void putbin(int v)
{
	if(!outpass || !bopt || !hexopt) return;
	*outbinp++ = v;
...
void eject()
{
	...
	if (outpass && !continuous_listing && iflist()) {
		if (popt) {
			putc('\014', fout);	/* send the form feed */
		} else {
			while (line < 65) {
...
for (;;) switch(charclass[c = nextchar()]) {
	...
	spectok:
		...
		*p = '\0';
		...
		// mzmac do not exclude VERBs in column 0
...
void version()
{
	fprintf(stderr, "mzmac version " VERSION "\n");
}
...
void usage(char *msg, char *param)
{
	...
	fprintf(stderr, "\n");
	version();
	fprintf(stderr, "usage: mzmac [-8bcdefghijJlLmnopstz] [-I dir] file[.z]\n");
	fprintf(stderr, "other opts: --rel --mras --dep --help --html --version\n");
/* 	fprintf(stderr, "other opts: --rel --mras --zmac --dep --help --doc --version\n"); */
	fprintf(stderr, "  mzmac -h for more detail about options.\n");
	exit(1);
}
...
void help()
{
	...
	fprintf(stderr, "\t-c\tadd cycle counts in listing\n");
	fprintf(stderr, "\t-d\tmake the listing discontinuous.\n");
	...
	fprintf(stderr, "\t-x\tgenerate listing to `listfile' (`-' for stdout)\n");
	fprintf(stderr, "\t-z\tuse Z-80 interpretation of mnemonics\n");

	fprintf(stderr, "\n\t--bin\toutput .bin file (default)\n");
	fprintf(stderr, "\t--ams\toutput .ams file\n");
	fprintf(stderr, "\t--bds\toutput .bds file\n");
	fprintf(stderr, "\t--cas\toutput .cas file\n");
	fprintf(stderr, "\t--cmd\toutput .cmd file\n");
	fprintf(stderr, "\t--hex\toutput .hex file\n");
	fprintf(stderr, "\t--lcas\toutput .lcas file\n");
	fprintf(stderr, "\t--rel\toutput .rel file\n");
...
/* 	fprintf(stderr, "\t--zmac\tcompatibility with original zmac\n"); */
	...
	fprintf(stderr, "\t--html\toutput detailed help in HTML format\n");
...
/*		if (strcmp(argv[i], "--zmac") == 0) {
		...
		if (*argv[i] == '-') while (*++argv[i]) {
			...
			case 'c':	/*  cycle counts in listing */
				copt++ ;
				continue;
			...
			case 'x':	/*  specify listing file */
				...
				if (i < argc)
					...
					usage("missing argument to -x option", 0);
				if(strcmp(listf, "-") == 0)
					oopt++;
				break;
	...
	if (
		...
		binopt++;		// default in absence of user selection
	...
	if (bopt) {
		...
		if (binopt) {
			outpath(oth, sourcef, ".bin");
			fbin = fopen(oth, "wb");
			if (fbin == NULL)
				error("Cannot create .bin file");
		}
...
/* 					ip->i_token = UNDECLARED; */
...
#endif
		fprintf(stderr, "Warning: no END entry address\n");
		fflush(stderr);
#if WIN32
		SetConsoleTextAttribute(hOut, inf.wAttributes);
#endif
	}
	else if (xeq_flag && bopt && bdsopt) {
		fprintf(fbds, "%04x e\n", xeq);
	}
	...
	if (bopt) {
		...
		if ((high >= low) && binopt)
			fwrite(memory + low, high + 1 - low, 1, fbin);
...
void putsymtab()
{
	...
	int  i, j, k, t, rows;
	char c, c1, seg = ' ';
	int numcol = printer_output ? 4 : 3;

	if (relopt)
		numcol = printer_output ? 4 : 1;
 ...
 */
 ...
 // Moved here instead than in mio.c since they ARE a need for macro file
 // P. betti
