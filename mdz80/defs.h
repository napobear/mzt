...
struct cycle_range {
	struct cycle_range * next;	// pointer to the next range in the same level
					// (NULL = no more ranges in this level)
	struct cycle_range *child; 	// pointer to the first child (NULL = no children)
	struct cycle_range *parent;	// pointer back to parent (NULL = this is a top level range)
...
typedef struct cycle_range	CYCLE_RANGE;
