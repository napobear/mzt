 ...
 */
/*
 * 20080220 -- porting in MDZ80
 *
 * AS part of MDZ80 i need an full cross-assembler. The goal is to have
 * generation of addresses directly from equates table or full asm sources.
 * In need of this i take zmac core source (zmac is a *beatiful* cross
 * assembler for the Z80/8080) and with minor modification have included
 * it in MDZ80.
 * Now the whole zmac act as an assembler/parser subsystem that, given an
 * appropriate input file, generate a series of directives for MDZ80
 * control files....
 *
...
static int	pass2;		/* set when pass one completed */
static int	dollarsign ;	/* location counter */
static int	olddollar ;	/* kept to put out binary */

/* program counter save for PHASE/DEPHASE */
static int	phdollar, phbegin, phaseflag ;

static char	*src_name[NEST_IN] ;
static int	linein[NEST_IN] ;
static int	now_in ;
...
static char	err[FLAGS];
static int	keeperr[FLAGS];
static char	errlet[FLAGS]="BEFIMPUVORZG";
static char	*errname[FLAGS]={
		"Balance",
		"Expression",
		"Syntax",
		"Digit",
...
static char	*lineptr;
static char	*linemax = linebuf+LINEBUFFERSIZE;

static char	outbin[BINPERLINE];
static char	*outbinp = outbin;
static char	*outbinm = outbin+BINPERLINE;

static char	emitbuf[EMITBUFFERSIZE];
static char	*emitptr;

static char	ifstack[IFSTACKSIZE];
static char	*ifptr;
static char	*ifstmax = ifstack+IFSTACKSIZE-1;


static char	expif[MAXIFS];
static char	*expifp;
static char	*expifmax = expif+MAXIFS;

static char	hexadec[] = "0123456789ABCDEF" ;
static char	*expstack[MAXEXP];
static int	expptr;
...
static char	*tempmax = tempbuf+TEMPBUFSIZE-1;

static char	inmlex;
static char	arg_flag;
static char	quoteflag;
static int	parm_number;
static int	exp_number;
static char	symlong[] = "Symbol too long";
...
static char	*title;
static char	titlespace[TITLELEN];
static char	*timp;
static char	*sourcef;
static char	*ctl;

/* changed to cope with filenames longer than 14 chars -rjm 1998-12-15 */
static char	srcf[1024];
static char	bin[1024];
static char	mtmp[1024];
static char	listf[1024];
static char	writesyms[1024];

static char	bopt = 0,			/* no binary output */
		edef = 0,			/* no error (define) report */
		eopt = 0,			/* no error report */
		fdef = 0,			/* no skipped line report */
		fopt = 0,			/* ... same */
		gdef = 1,			/* extra code listing */
		gopt = 1,			/* ... same */
		iopt = 0 ,			/* list include files */
		lstoff = 0,
		lston = 0,			/* flag to force listing on */
		lopt = 1,			/* no listing */
		mdef = 0,			/* macro expansion */
		mopt = 0,			/* ... same */
		nopt = 1,			/* line numbers on as default */
		oldoopt = 0,			/* std output listing */
...
static int	had_errors = 0;		/* if program had errors, do exit(1) */
static int	not_seen_org = 1;
static int	first_org_store = 0;
...
int addtoline(int ac)
{
	...
	if (inmlex)
		return(ac);
	if (lineptr >= linemax)
		zmacerror("line buffer overflow");
	*lineptr++ = ac;
	return(ac);
}
...
void emit(int bytes, ...)
{
	...
	while	(--bytes >= 0)
		if (emitptr >= &emitbuf[EMITBUFFERSIZE])
			zmacerror("emit buffer overflow");
		else {
...
void flushbin()
{
	...
	if (check) {
		if (output_hex) {
			putc(':', fbuf);
			zm_puthex(check, fbuf);
			zm_puthex(olddollar>>8, fbuf);
			zm_puthex(olddollar, fbuf);
			zm_puthex(0, fbuf);
		}
 ...
 */
void zm_puthex(char byte, FILE *buf)
{
...
void list(int optarg)
{
	...
	if (pass2) {
		...
		if (lst) {
			...
			zm_puthex(optarg >> 8, fout);
			zm_puthex(optarg, fout);
			fputs("  ", fout);
...
al:
	{ int i;
		if (expptr >= MAXEXP)
			zmacerror("Macro expansion level");
...
loop switch(charclass[c = nextchar()]) {
	...
	default:
		...
		case '"':
			 ...
			 *  our input line
			 */
			zmacerror("string buffer overflow");
		case '<':
			corig = c;
			switch (c = nextchar ()) {
			case '=':
...
int zmac(char * srcfil, char * ctlnam, int dobin, int dolist)		// ... this was main()
{
	struct item *ip;
	int  i, c;
	...
	*bin = *listf = 0;
	...
	if (dobin) bopt++;		// mmmh, an embedded assembler will work...
	if (dolist) lopt = 0;		// and listing too

	if ((now_file = fopen(srcf, "r")) == NULL)
	{
		/* If filename has no pre-existing suffix, then try .z */
		suffix_if_none (srcf, "z");
		if ((now_file = fopen(srcf, "r")) == NULL)
			fileerror("Cannot open source file", srcf);
	}
	now_in = 0 ;
	fin[now_in] = now_file ;
	src_name[now_in] = srcf ;
	...
	if (bopt) {
		...
		if (output_hex) {
			...
			if (xeq_flag) {
				zm_puthex(0, fbuf);
				zm_puthex(xeq >> 8, fbuf);
				zm_puthex(xeq, fbuf);
				zm_puthex(1, fbuf);
				zm_puthex(255-(xeq >> 8)-xeq, fbuf);
			} else
				for	(i = 0; i < 10; i++)
					putc('0', fbuf);
			putc('\n', fbuf);
	...
	if (!lopt)
		fflush(fout);
	/* symbols table output (wsym directive) unconditionally disabled */
//		if (*writesyms)
//			outsymtab(writesyms);
	if (eopt)
		erreport();
...
/*
 *  output the symbol table (to control file....)
 */
void putsymtab()
{
	struct item *tp, *fp;
...
/* The original version of putsymtab ... */
...
void putctltab()
{
	...
	sprintf(comment, "\t; from %s", basename(srcf));
	...
	for (k = 0; k < nitems; k++) {
		tp = &itemtab[k];
		if (tp->i_value < 0x10)		/* single digit equs generates too much noise */
			continue;
		if (tp->i_token == EQUATED || tp->i_token == DEFLED) {
			fprintf(ctlfp, "s %4x\t%s\t%s\n",
			               ((tp->i_value > 0xff) ? tp->i_value & 0xffff : tp->i_value & 0xff),
			               tp->i_string,
			               comment
			       );
		}
			...
			if (tp->i_value > 0xff) {
				fprintf(ctlfp, "a %4x\t\t\t%s\n",
				               tp->i_value & 0xffff,
				               comment
				       );
			}
			fprintf(ctlfp, "l %4x\t%s\t%s\n",
			               ((tp->i_value > 0xff) ? tp->i_value & 0xffff : tp->i_value & 0xff),
			               tp->i_string,
			               comment
			       );
...
int getarg()
{
	...
	*tempbuf = 0;
	...
	case '\'':
		...
		zmacerror(symlong);		/* doesn't return */
	 ...
	 * -Wall... -rjm */
	zmacerror("can't happen - in zmac.y:getarg(), infinite unquoted string!?");
	return(0);
}
...
void insymtab(char *name)
{
	...
	if ((sfile = fopen(name, "rb")) == NULL) {
		fprintf(stderr,"Cannot open sym file: [%s]\n", name);
		return;
	}
...
void next_source(char *sp)
{
	...
	if(now_in == NEST_IN -1)
		zmacerror("Too many nested includes") ;
