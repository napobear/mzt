...
struct object_item {
	...
	union {
		u_int8_t absolute_byte;
		u_int16_t relative_word;
		struct {
			int32_t control;
...
#define MARKED32(a)	(((u_int32_t*)usage_map)[(a)/32] == 0xffffffff)
#define UNMARKED32(a)	(((u_int32_t*)usage_map)[(a)/32] == 0)
...
struct zsymbol *find_zsymbol ( char * );
int add_zsymbol ( char *, int, struct section * );
struct zsymbol *get_zsymbol ( char * );
int init_zsymbol ( int );
void clear_zsymbol ( void );
void dump_zsymbols ( void );
void set_zsymbols ( void );
void print_zsymbol_table ( FILE * );
