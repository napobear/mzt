...
int main ( int argc, char **argv ) {
	...
	int abort = 0;
	int lib = 0;
	int zsymbol_table_required = 0, map_required = 0;
	char *common_name = "";
...
#define	REGULAR_OPTSTRING	"-VlP:D:C:U:O:o:hcs:mS:W:g"
#ifdef	DEBUG
...
#define	OPTSTRING		REGULAR_OPTSTRING
		...
		switch ( c ) {
			...
			case 'U':	/* "Uncommon" segment */
...

				{
					...
					for ( s = optarg; *s; s++ ) *s = toupper ( *s );
				}
				mark_uncommon ( optarg );
				break;

			case 'S':	/* Set zsymbol table size */
				n = atoi ( optarg );

				if ( n <= 0 ) {
					fprintf ( stderr, "ld80: zsymbol table size is invalid\n" );
					abort = 1;
					break;
				}
	...
	if ( zsymbol_table_required || map_required ) {
		if ( !symfilename || !strcmp ( symfilename, "-" ) ) {
			...
			if ( symfile == NULL )
				die ( E_USAGE, "ld80: Cannot open zsymbol file %s: %s\n",
	 ...
	 */
	init_zsymbol ( symsize );

	optind = 0;	/* make reinitialize getopt() */

	while ( ( c = getopt ( argc2, argv2, "-lD:P:C:" ) ) != -1 )
		switch ( c ) {
...
#define	IFDEBUG(x)
	...
	IFDEBUG ( printf ( "\nSetting zsymbol values\n" ); )
	set_zsymbols();
	IFDEBUG ( dump_zsymbols(); )
	...
	if ( zsymbol_table_required ) print_zsymbol_table ( symfile );
