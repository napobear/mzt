...
void add_section ( int type, char *common_name,
	...
	if ( debug )
		printf ( "add_section(type=%s, %s, start=%.4x, len=%.4x, %s)=%p\n",
		         atypes[type], type == T_COMMON ? common_name : "<null>",
...
int to_chain ( struct object_item *item, char *filename ) {
	...
	if ( MARKED ( 0 ) && MARKED ( 1 ) ) {
		fprintf ( stderr,
		          ...
		          "* There is no way to figure out if value of external zsymbol %s\n"
		...
		fprintf ( stderr,
		          ...
		          "* to external zsymbol %s, but actually does not use it.\n"
		          "* \n",
...
void add_item ( struct object_item *item, char *filename ) {
	...
	switch ( item->type ) {
		...
		case T_ABSOLUTE:
			...
			if ( p->lc >= p->len ) die ( E_INPUT, "ld80: Segment overloaded lc=%d, len=%d\n", p->lc, p->len );
			...
			if ( current_section_t == T_ABSOLUTE ) {
				if ( MARKED ( p->lc ) ) {
					if ( !overlap++ )
						fprintf ( stderr, "ld80: Overlapping ASEG in file %s\n", filename );
					fatalerror = 1;
				}
				else MARK_BYTE ( p->lc );
			}
		...
		case T_RELOCATABLE|T_SPECIAL:
			switch ( item->v.special.control ) {
				...
				case C_ENTRY_POINT:	/* 7 */
					add_zsymbol ( ( char * ) BNAME, AVALUE, secs[ATYPE] );
					break;
