	...
	for ( segp = segv; segp <= segv + T_COMMON || segp->secs; segp++ ) {
		for ( sp = segp->secs; sp; sp = sp->next ) { /* all sections */
			for ( n = sp->nodehead; n; n = n->next ) {
				...
				n->type = N_OPERAND;
				s = get_zsymbol ( n->zsymbol->name );

				if ( s == NULL || s->value == UNDEFINED ) {
					fprintf ( stderr,
					          "ld80: Undefined zsymbol %s\n",
					          n->zsymbol->name );
					undefined++;
					fatalerror = 1;
					n->value = 0;
...
// n->at.offset, n->type, n->value, n->zsymbol ? n->zsymbol->name : "");
				switch ( n->type ) {
					case N_OPERAND:
						if ( n->at.section )
...
//if (tos) printf("tos: at=%p:%.4x type=%d value=%.4x zsymbol=%s\n", tos->at.section,
//tos->at.offset, tos->type, tos->value, tos->zsymbol ? tos->zsymbol->name : "");
//else printf("empty\n");
			} /* while */

			if ( tos ) {
				die ( E_INPUT, "ld80: Expression evaluation error\n" );
//printf("tos: at=%p:%.4x type=%d value=%.4x zsymbol=%s\n", tos->at.section,
//tos->at.offset, tos->type, tos->value, tos->zsymbol ? tos->zsymbol->name : "");
			}
		} /* for for */
}
